    ...
    def validate(self, password, user=None):
        if len(password) < self.min_length:
            raise ValidationError(
                ungettext(
                    "비밀번호가 너무 짧습니다. %(min_length)d 자 이상 입력하세요.",
                    "비밀번호가 너무 짧습니다. %(min_length)d 자 이상 입력하세요.",
                    self.min_length
                ),
        ...
        for attribute_name in self.user_attributes:
            ...
            for value_part in value_parts:
                if SequenceMatcher(a=password.lower(), b=value_part.lower()).quick_ratio() >= self.max_similarity:
                    ...
                    raise ValidationError(
                        _("%(verbose_name)s와 너무 유사합니다."),
                        code='password_too_similar',
                        params={'verbose_name': verbose_name},
                    )

    def get_help_text(self):
        return _("인적사항과 유사한 비밀번호를 사용하실 수 없습니다.")
...
class CommonPasswordValidator(object):
    ...
    def get_help_text(self):
        return _("너무 흔한 비밀번호를 사용하실 수 없습니다.")
...
class NumericPasswordValidator(object):
    ...
    def validate(self, password, user=None):
        if password.isdigit():
            raise ValidationError(
                _("비밀번호가 숫자로만 구성되어 있습니다."),
                code='password_entirely_numeric',
            )

    def get_help_text(self):
        return _("숫자로만 구성된 비밀번호를 사용하실 수 없습니다.")
